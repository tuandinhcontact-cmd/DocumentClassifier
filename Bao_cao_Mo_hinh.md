...
| **3-Step Cascade (Post-merge Cap 40k - Phân rã nhãn News - ƯU VIỆT NHẤT)** | **117,983** | **74.26%** | **59.97%** | **4.01s** |

**Nhận xét phân tích kết quả:**
* **Kịch bản Phân rã nhãn News (Ưu việt nhất):** Việc xóa bỏ hoàn toàn nhãn nhiễu `News` (F1-score cũ chỉ là 0.20) và tái phân bổ các nhãn con gốc của nó về các chủ đề chuyên biệt hơn đã đem lại hiệu quả đột phá. **Test Accuracy tăng vọt lên 74.26% (tăng +1.99%)** và **Macro F1-Score đạt mốc kỷ lục là 59.97% (tăng mạnh +2.84%)**. Sự thay đổi này giúp mô hình không còn bị nhầm lẫn ngữ nghĩa rộng của nhãn `News`, đồng thời củng cố dữ liệu và nâng cao F1-score của các lớp liên quan như `Politics and society` (F1 tăng từ 0.80 lên 0.82), `Education` (F1 tăng từ 0.42 lên 0.45), và `Health` (F1 tăng từ 0.46 lên 0.48).

...
def apply_post_merge_capping(df, target_cap):
    sampled_groups = []
    for cat_name, group in df.groupby("category"):
        if cat_name in ["Tech & Science", "Politics and society"]:
            sampled_group = group.sample(
                n=min(len(group), target_cap),
                random_state=42
            )
            sampled_groups.append(sampled_group)
        else:
...
def main():
    ...
    # POST-MERGE CAPPING: Cap Tech and Politics to 50,000 samples
    merged_df = apply_post_merge_capping(merged_df, 50000)
    
    print(f"Total dataset size after post-merge capping (Tech/Pol = 50k): {len(merged_df)} rows")
    ...
    print(f"\nRESULTS FOR POST-MERGE CAPPING (50k):")

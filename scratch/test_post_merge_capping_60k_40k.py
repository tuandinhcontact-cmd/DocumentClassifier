...
def apply_post_merge_capping(df, tech_cap, pol_cap):
    sampled_groups = []
    for cat_name, group in df.groupby("category"):
        if cat_name == "Tech & Science":
            sampled_group = group.sample(
                n=min(len(group), tech_cap),
                random_state=42
            )
            sampled_groups.append(sampled_group)
        elif cat_name == "Politics and society":
            sampled_group = group.sample(
                n=min(len(group), pol_cap),
                random_state=42
            )
            sampled_groups.append(sampled_group)
        else:
...
def main():
    ...
    # POST-MERGE CAPPING: Cap Tech to 60k and Politics to 40k samples
    merged_df = apply_post_merge_capping(merged_df, 60000, 40000)
    
    print(f"Total dataset size after post-merge capping (Tech=60k, Pol=40k): {len(merged_df)} rows")
    ...
    print(f"\nRESULTS FOR POST-MERGE CAPPING (Tech=60k, Pol=40k):")
